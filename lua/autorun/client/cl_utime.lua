...
local function buildCP( cpanel )
    ...
    cpanel:AddControl( "Header", { Text = "UTime" } )
...
local function onEntCreated( ent )
    if LocalPlayer() and LocalPlayer():IsValid() and utime_outsidecolor_r and utime_outsidecolor_r:GetInt() == 256 then
        resetCvars()
        hook.Remove( "OnEntityCreated", "UTimeLocalPlayerCheck" )
    end
end
hook.Add( "OnEntityCreated", "UTimeLocalPlayerCheck", onEntCreated )